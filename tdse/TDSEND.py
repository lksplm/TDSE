...
class TDSEND_solver(object):
    def __init__(self, coords, dt, tol=1.0e-4, psi0=None, V0=None, Vtd = [], m=1., g=0.):
        """
        ...
        self._VsTimeDep = Vtd
        self._V0dynamic = np.zeros(self._N, dtype=np.complex64)
        
        self._m = m                                              # particle mass
        self._tol = tol
        self._t = 0                                             # time propagated
        self._g = g                     #gp coefficient
        ...
        #evaluate time-dependent potential at t=0
        self.evalTdPot()
    ...
    def _x_step(self, psi=None, dt=None, imag=False):
        ...
        if imag:
            ...
            #update time-dependent potentials
            self.evalTdPot()
    ...
    def addPotentialTd(self, func, **kwargs):
        self._VsTimeDep.append(func)
        
    def evalTdPot(self):
        if len(self._VsTimeDep)>0:
            self._V0dynamic = np.zeros(self._N, dtype=np.complex64)
            for p in self._VsTimeDep:
                self._V0dynamic += p.compute(*self._grid, self._t)
        
            self.gpu_V0 = gpuarray.to_gpu_async(self._V0 + self._V0dynamic)
    ...
    def getV0(self):
        return np.real(self._V0+self._V0dynamic)

...
class moving_well:
    def __init__(self, d, w, dmin, t_dur, t_0, t_1, std=1., pert=0., dist=3):
        self.d = d
        self.w = w
        self.dmin = dmin
    ...
    def compute(self, x, t):
        ...
        pot = g(x, self.w, 0)+(-1)*xL*g(x, self.w, -self.dist)+xR*g(x, self.w, self.dist)
        return pot
    ...
    def plot(self, ax=None, save=False):
        ...
        plt.legend()
        if save:
            plt.savefig("potential_depth.png", dpi=300)
        plt.show(block=True)
...
movingPot = moving_well(d=1., w=waist, dmin=0.5, t_dur=600, t_0=62, t_1=0., dist=5.0, pert=0.0)
movingPot.plot()

Simulation = TDSES.TDSEND_solver(coords=gridsize, dt=1.0e-4, tol=1.0e-4, Vtd = [movingPot,])#, 
Simulation.addCAP()
Simulation.setWavefunction(gauss1d, mx=-5.0,  sigx=waist)
